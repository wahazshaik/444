...
- Initialize your logger object by calling the 'init_logging' function *(with mailing feature turned OFF)*
        ...
        my_logger_obj = init_logging(log_name='my_logs', log_directory='logs_dir', enable_mailing=False)

- Initialize your logger object by calling the 'init_logging' function *(with mailing feature turned ON)*

        from logging_essar import init_logging
        my_logger_obj = init_logging(log_name='my_logs', log_directory='logs_dir', mailfrom_addr='<email_address>', 
...
def check_params(**kwargs):
    ...
    if kwargs.get('enable_mailing'):
        ...
        if not isinstance(kwargs.get('mail_credentials'), tuple):
            raise ArgumentError('Invalid MailCredentials')
        else:
...
def init_logging(log_name='logger', log_directory='logs', log_mode='a', max_bytes=100 * 1024 * 1024,
                 ...
                 log_format='[%(asctime)s] -- %(levelname)s - %(filename)s -- %(funcName)s - Line no - %(lineno)d '
                            '-- %(message)s\n', enable_mailing=True, mail_host='email.essar.com', mailfrom_addr=None,
                 mailto_addr=None, mail_subject='Exception Report', mail_credentials=None, mail_secure=None,
                 mail_timeout=2.0):
    """
